...
export default function DealsHero() {
  ...
  const [form, setForm] = useState<SearchForm>({
    pickupLocation: "Split, Croatia",
    returnLocation: "Split, Croatia",
    pickupDate: "",
    pickupTime: "",
    dropoffDate: "",
    dropoffTime: "",
  ...
  const [errors, setErrors] = useState<
    Partial<Record<keyof SearchForm, string>>
  >({});
  ...
  function onSubmit(e: React.FormEvent) {
    ...
    if (!result.success) {
      ...
      const fieldErrors: Partial<Record<keyof SearchForm, string>> = {};
      result.error.issues.forEach((err) => {
        if (err.path[0]) {
          fieldErrors[err.path[0] as keyof SearchForm] = err.message;
        }
      });
    ...
    // Clear errors and proceed with navigation
    setErrors({});
  ...
  return (
    <section className={`${styles.hero} w-full`}>
      <div className="container mx-auto px-4">
        ...
        <form
          onSubmit={onSubmit}
          className={`${styles.glass} mt-8 p-5 sm:p-6 grid gap-4 sm:gap-6 grid-cols-1 md:grid-cols-3`}
        >
          <div className="flex flex-col">
            <label
              className={`${styles.fieldLabel} mb-2 flex items-center gap-2`}
            >
